...
// View at https://supabase.com/dashboard/project/trtqbruuzdvlmzrzwrot/database/tables
...
export const mset = async (keys: string[], values: any[]): Promise<void> => {
  const supabase = client()
  const { error } = await supabase.from("kv_store_5cb00c7d").upsert(keys.map((k, i) => ({ key: k, value: values[i] })));
  if (error) {
    throw new Error(error.message);
  }
};
...
export const mget = async (keys: string[]): Promise<any[]> => {
  const supabase = client()
  const { data, error } = await supabase.from("kv_store_5cb00c7d").select("value").in("key", keys);
  if (error) {
    throw new Error(error.message);
  }
...
export const mdel = async (keys: string[]): Promise<void> => {
  const supabase = client()
  const { error } = await supabase.from("kv_store_5cb00c7d").delete().in("key", keys);
  if (error) {
    throw new Error(error.message);
  }
};
